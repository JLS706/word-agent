...
7. **用户画像人类独占**：你绝对不能自行决定更新用户画像（update_profile），\
必须在用户明确表达长期偏好时才能提议，且必须获得用户确认后才能执行。\
你的反思和总结应存入 L2 长期记忆，而非 L1 用户画像。
...
6. **用户画像人类独占**：不能自行 `update_profile`，必须用户明确确认后才能执行
...
def _load_l1_sections() -> tuple[str, str]:
    """
    加载 L1 内容，返回 (头部注入文本, 尾部重复文本)。
    优先使用 user_profile.md（新版画像），回退到 learned_rules.json（旧版铁律）。
    """
    learned_section = ""
    reminder = ""
    try:
        from tools.learned_rules import (
            load_profile_for_prompt, extract_taboos_from_profile,
            load_rules_for_prompt, _load_rules,
        )
        # 优先尝试新版画像
        learned_section = load_profile_for_prompt()
        if learned_section:
            # 从画像中提取铁律做尾部 reminder（首尾夹击）
...
def build_worker_prompt(
    ...
    if role_key == "planner":
        ...
        # Reviewer 需要 L1 铁律段落（优先画像，回退 JSON）
        l1_section = ""
        try:
            ...
            if taboos:
                rule_lines = "\n".join(
                    f"{i}. **{t}**" for i, t in enumerate(taboos, 1)
                )
            else:
                rules = _load_rules()
                rule_lines = "\n".join(
...
def build_l1_user_suffix(task_text: str = "") -> str:
    ...
    三明治注入法：将 L1 铁律 Append 到用户输入的末尾（紧贴生成起始点），
    利用 LLM 的近因效应（Recency Bias）最大化规则服从度。

    优先从 user_profile.md 的 ## 铁律 section 提取，回退到旧 JSON rules。
    如果 task_text 非空，启用策略四（动态 RAG），只注入与当前任务相关的规则。
    """
    try:
        # 优先尝试从画像提取铁律
        from tools.learned_rules import extract_taboos_from_profile
        taboos = extract_taboos_from_profile()
        if taboos:
            # 策略四：动态挂载 — 只注入相关铁律
            if task_text:
                # 将 taboos 转为旧格式兼容 select_relevant_rules
                pseudo_rules = [{"rule": t} for t in taboos]
                relevant = select_relevant_rules(pseudo_rules, task_text)
                taboo_strs = [r["rule"] for r in relevant]
            else:
                taboo_strs = taboos

            if taboo_strs:
                return f"\n\n[⚠️ 铁律] {'；'.join(taboo_strs)}"
            return ""
